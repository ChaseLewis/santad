...
import type { ThemeConfig, ResolvedTheme, ResolvedThemeMode } from './types';
 ...
 */
function generateDerivedTokens(
  mode: ResolvedThemeMode,
  tokens: typeof seedTokens
): ResolvedTheme {
 ...
 */
export function createDefaultTheme(config?: ThemeConfig): ResolvedTheme {
  // Handle 'system' mode - defaults to 'light' if not resolved by caller
  const mode: ResolvedThemeMode = config?.mode === 'system' || !config?.mode ? 'light' : config.mode;
 ...
 */
export function setThemeContext(theme: ResolvedTheme): void {
 ...
 */
export function getThemeContext(): ResolvedTheme {
  const ctx = getContext<{ current: ResolvedTheme } | ResolvedTheme>(THEME_CONTEXT_KEY);
  if (!ctx) return createDefaultTheme();
  // Handle both reactive wrapper and direct theme
  if ('current' in ctx) return ctx.current;
  return ctx;
}
 ...
 */
export function themeToCSSVars(theme: ResolvedTheme, prefix = 'ant'): Record<string, string> {
