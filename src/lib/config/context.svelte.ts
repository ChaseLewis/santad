import { getContext, setContext } from 'svelte';
import type { ResolvedConfig } from './types';
import { defaultConfig } from './types';

const CONFIG_CONTEXT_KEY = Symbol.for('santad-config');

/**
 * Set config in Svelte context
 */
export function setConfigContext(config: ResolvedConfig): void {
  setContext(CONFIG_CONTEXT_KEY, config);
}

/**
 * Get config from Svelte context
 */
export function getConfigContext(): ResolvedConfig {
  return getContext<ResolvedConfig>(CONFIG_CONTEXT_KEY) ?? defaultConfig;
}

/**
 * Create a getPrefixCls function with the given base prefix
 */
export function createGetPrefixCls(basePrefixCls: string) {
  return (suffixCls?: string, customizePrefixCls?: string): string => {
    if (customizePrefixCls) return customizePrefixCls;
    return suffixCls ? `${basePrefixCls}-${suffixCls}` : basePrefixCls;
  };
}

